...
export class UniswapV2Factory {
   ...
   */
  async getPairAddress(wallet: Wallet, tokenAddress: string): Promise<string> {
    this.factoryContract = this.factoryContract.connect(wallet) as Contract;
  ...
  async getPair(wallet: Wallet, tokenAddress: string): Promise<UniswapV2Pair> {
    const pairAddress = await this.getPairAddress(wallet, tokenAddress);
    if (!pairAddress) {
