...
describe("Aerodrome Strategy Test", () => {
  ...
  beforeAll(async () => {
    ...
    strategy = new AerodromeStrategy(chain);
  });
  ...
  describe("constructor and basic setup", () => {
    ...
    it("should initialize with correct chain configuration", () => {
      const testStrategy = new AerodromeStrategy(ChainType.BASE);
      expect(testStrategy.getName()).toBe("TestStrategy");
    });

    it("should throw error when initialized on non-Base chain", () => {
      expect(() => new AerodromeStrategy(ChainType.ETH)).toThrow("AerodromeStrategy is only supported on Base chain");
    });

    it("should throw error when initialized on Arbitrum chain", () => {
      expect(() => new AerodromeStrategy(ChainType.ARB)).toThrow("AerodromeStrategy is only supported on Base chain");
    });
  });
  ...
  describe("createTransaction", () => {
    ...
    it("should handle different output tokens", async () => {
      ...
      const tx = await strategy.createTransaction(cbethTrade, wallet);
  ...
  describe("Aerodrome-specific functionality", () => {
    ...
    it("should apply proper slippage in createTransaction", async () => {
      // The createTransaction method uses 5% slippage (95% of expected output)
      const tx = await strategy.createTransaction(ethToTokenTrade, wallet);
...
describe("Cross-Chain validation", () => {
  ...
  it("should work correctly on Base chain", () => {
    const strategy = new AerodromeStrategy(ChainType.BASE);
